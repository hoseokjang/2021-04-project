...
<style>
	...
	#prod_category
	{
		padding-top:20px;padding-bottom:20px;padding-left:200px;
		margin:15px;
	...
	#category_left
	{
		display:table-cell;width:190px;
		padding-top:20px;padding-bottom:20px;padding-left:3px;
		border-bottom:1px solid grey;
	}
	#prod_list
	{
		display:table-cell;width:930px;
	...
	#prod_page a{padding-left:4px;padding-right:4px;}
</style>
...
%>
<!-- 문서 시작 -->
<div id="third_prod_list" align="center" style="width:1900px"> <!-- 전체 문서의 시작 -->
...
<div id="category_left" align="left"> <!-- 왼쪽 div 시작 -->
	...
	<div id="price_search" align="left" style="border-top:1px solid black;"> <!-- 가격 별로 보여주기 -->
		...
		<div id="price_price_search_list">
			...
			<a href="prod_list.jsp?prod_category=<%=rs2.getString("prod_category") %>&min_prod_price=50000&max_prod_price=300000">5만원~30만원</a>
		</div>
	</div>
</div> <!-- 왼쪽 div끝 -->
...
<div> <!-- 페이징 처리 시작 -->
	...
	<%
		...
		int pend= pstart+9;
		int cnt = pdao.prod_cnt(request, rs2.getString("prod_category"));
		int page_cnt = cnt/20;
		if (cnt%10 != 0)
			page_cnt++;
	...
	%>
		<%if(ppage != 1){ %>
			<a href="prod_list.jsp?prod_category=<%=rs2.getString("prod_category") %>&ppage=<%=(ppage-1) %>">◁</a>
		<%}
		String str = null;
		for (int i = pstart; i<=pend; i++)
		{
			if (ppage == i)
				...
				str ="";
		%>
				<a href="prod_list.jsp?prod_category=<%=rs2.getString("prod_category") %>&ppage=<%=i %>" <%=str %>> <%=i %> </a>
		<%
		/*	if (i==page_cnt)
				break;
		*/
